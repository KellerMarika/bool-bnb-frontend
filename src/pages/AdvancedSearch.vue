<template>
	<div class="container">
		<form @submit.prevent="fetchTomTom">
			<div
				class="d-flex align-items-center justify-content-around gap-4 p-5">
				<div class="input-container mb-2 col-3">
					<label class="form-label ms-2" for="city">Street and City</label>
					<input
						type="text"
						placeholder="Ex. Via generale cascino 14 Roma"
						class="form-control"
						id="streetNameInput"
						v-model="address" />
				</div>
				...
				<div class="input-container pb-2 col-3 text-center">
					<label class="form-label" for="min_rooms"
						>min Rooms number</label
					>
					<select
						v-model="query.min_rooms"
						class="form-control"
						id="min_rooms"
						name="min_rooms">
				...
				<div class="mb-2 col-3 text-end">
					<label class="form-label me-2" for="radiusInput"
						>Radius (km)</label
					>
					<select
					...
					<div class="row">
						<div
							...
							<label for="service_{{i}}"
								>{{
									service.name == 'Aria Condizionata'
										? 'Clima'
										: service.name
								}}
							</label>
